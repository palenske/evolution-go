...
func (s *sendService) ensureClientConnected(instanceId string) (*whatsmeow.Client, error) {
	client := s.registry.GetClient(instanceId)
...
func (s *sendService) SendMessage(instance *instance_model.Instance, msg *waE2E.Message, messageType string, data *SendDataStruct) (*MessageSendStruct, error) {
	...
	// Resolve o client uma unica vez. Antes, cada uso relia o map compartilhado e
	// desreferenciava sem checagem: se a instancia caisse no meio do envio, o
	// processo entrava em panico com nil pointer.
	client := s.registry.GetClient(instance.Id)
	if client == nil {
		s.loggerWrapper.GetLogger(instance.Id).LogError("[%s] No active client for instance", instance.Id)
